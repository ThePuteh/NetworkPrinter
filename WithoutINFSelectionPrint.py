...
def choose_printer():
    while True:
        ...
        if var_choice == "1":
            ...
            var_inf_path = r"C:\Printer\CanonPCL6\CNP60MA64.INF"
            ...
            var_inf_path = r"C:\Printer\HPPCL6\HPPCL6.INF"
            confirm_installation(var_choice, var_brand, var_driver, var_ip, var_inf_path)
            break
        else:
        ...
        if var_confirm in ["YES", "NO", "RESTART", "Y", "N", "R"]:
            if var_confirm == "YES" or var_confirm == "Y":
                ...
                if os.path.exists(rundll32_path):
                 # subprocess.run([rundll32_path, "printui.dll,PrintUIEntry", "/ia", "/f", var_inf_path, "/h", "x64"], shell=True)                                               # Shellcode Injection occur here due to shell=true
                 # subprocess.run([rundll32_path, "printui.dll,PrintUIEntry", "/if", "/b", "Network Printer", "/f", var_inf_path, "/r", var_ip, "/m", var_driver], shell=True)   # Shellcode Injection occur here due to shell=true
